 ...
 * Checks the correct number and format of arguments have been parsed and prints the usage to stderr otherwise.
...
int main(int argc, char* argv[]) {
    // Print the usage if an incorrect quantity of arguments are given.
    if (argc != 5) {
        fprintf(stderr, "Usage: %s <input_image> <output_image> <num_colours> <num_iterations>\n", argv[0]);
        return EXIT_FAILURE;
    }

    // Assign the image path arguments.
    const char* input_image = argv[1];
    const char* output_image = argv[2];

    // Convert the colour quantity to an integer and exit with an error if incorrectly formatted.
    int num_colours = atoi(argv[3]);

    if (num_colours <= 0) {
        fprintf(stderr, "Error: Number of colours to compress to must be a positive integer greater than zero.\n");
        return EXIT_FAILURE;
    }

    // Convert the iteration count to an integer and exit with an error if incorrectly formatted.
    int num_iterations = atoi(argv[4]);

    if (num_iterations <= 0) {
        fprintf(stderr, "Error: Number of iterations to perform must be a positive integer greater than zero.\n");
        return EXIT_FAILURE;
    }

    // Execute the compression on the parsed image.
    return compress_image(input_image, output_image, num_colours, num_iterations);
}
